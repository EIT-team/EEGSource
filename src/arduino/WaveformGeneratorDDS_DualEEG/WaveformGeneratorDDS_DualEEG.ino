...
int iSample_DAC1 = StartSample;
int iSample_DAC0 = StartSample + DAC0DELAY;
int iRep = 0;
...
void setup() {
	...
	Serial.begin(115200);
	Serial.println("hello, making EEG signals");
...
void TC3_Handler() //this is the ISR for the 500kHz timer - runs every 2 uS 
{
	...
	dacc_write_conversion_data(DACC_INTERFACE, waveformsTable[iSample_DAC0]);

	/* write to DAC1 */
	dacc_set_channel_selection(DACC_INTERFACE, 1);
	dacc_write_conversion_data(DACC_INTERFACE, waveformsTable[iSample_DAC1]);//write on DAC DACC_INTERFACE is currently *already* set to DAC1 from the original AnalogWrite call

	iSample_DAC0++;
	iSample_DAC1++;

	// if we have finished wavetable
	if (iSample_DAC0 == EndSample)
	{
		iSample_DAC0 = StartSample+ DAC0DELAY; //reset counter	
	}

	// if we have finished wavetable
	if (iSample_DAC1 == EndSample)
	{
		iSample_DAC1 = StartSample; //reset counter
		iRep++;
	}
...
void TC1_Handler() //this is the ISR for the 2Hz timer
{
	...
	iRep = 0;
	iSample_DAC1 = StartSample;
	iSample_DAC0 = StartSample + DAC0DELAY;
